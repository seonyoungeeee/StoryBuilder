    ...
    <div ref={ref} className="mx-auto w-full max-w-[794px] bg-paper p-8 md:p-12">
      ...
      <div className="mt-8 flex flex-col gap-6">
        {scenes.map((scene, index) => (
          <div key={scene.id} className="break-inside-avoid rounded-card border border-border bg-white/70 p-5">
            <div className="grid grid-cols-1 gap-4 md:grid-cols-[1fr_220px]">
              <div>
                <p className="font-mono text-xs font-semibold tracking-widest text-slate">
                  SCENE {String(index + 1).padStart(2, '0')}
                </p>
                <h2 className="mt-1 font-display text-lg font-bold text-ink">{scene.title}</h2>
                <p className="mt-2 whitespace-pre-wrap text-sm leading-relaxed text-ink-light">
                  {scene.content || '(내용 없음)'}
                </p>
              </div>
              <div className="flex items-center justify-center rounded-md border border-dashed border-ink/30 bg-white">
                {scene.sketchDataUrl ? (
                  <img
                  ...
                  <span className="p-6 text-center text-xs text-ink-faint">스케치 없음</span>
                )}
              </div>
            </div>
          </div>
